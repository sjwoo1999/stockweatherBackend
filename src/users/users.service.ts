...
export class UsersService {
  ...
  async getMockUserSummary(userId: number): Promise<any[]> {
    console.log(
      `UsersService: getMockUserSummary 호출됨 (사용자 ID: ${userId})`,
    );
    return [
      {
        date: '2025년 05월 24일',
  ...
  async getMockUserDetail(userId: number): Promise<StockData[]> {
    console.log(
      `UsersService: getMockUserDetail 호출됨 (사용자 ID: ${userId})`,
    );
    const mockStockData: StockData = {
      ...
      sentimentScore: 0.9,
      keywords: [
        { text: 'HBM', sentiment: 'POSITIVE' },
        { text: '파운드리', sentiment: 'NEUTRAL' },
      ],
      reportSummary:
        '삼성전자의 HBM 반도체 기술 발전과 AI 시장 확대로 긍정적인 투자 의견이 지배적입니다.',
      // ⭐ articles 필드 제거 ⭐
      detailedAnalysis: {
        positiveFactors:
          '최근 삼성전자는 HBM3E 개발 성공 소식과 함께 AI 반도체 시장에서의 입지를 강화하고 있습니다. 이는 장기적인 성장 동력으로 작용할 것입니다.',
        negativeFactors:
          '경쟁 심화와 글로벌 경기 둔화 가능성은 여전히 부정적인 요인으로 작용할 수 있습니다.',
        neutralFactors:
          '특정 뉴스에서 언급된 시장 동향이나 규제 변화 등은 단기적인 영향이 불확실합니다.',
        overallOpinion:
          '삼성전자는 HBM3E 개발과 AI 반도체 시장에서의 입지 강화로 긍정적인 성장 동력을 확보하고 있습니다. 그러나 경쟁 심화와 거시 경제 상황에 대한 지속적인 모니터링이 필요합니다.',
      },
