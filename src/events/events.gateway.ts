...
import {
  ...
  export class EventsGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    handleDisconnect(client: Socket) {
      this.logger.log(`[${EventsGateway.name}] Client disconnected: ${client.id}`);
    ...
    sendToClient(socketId: string, eventName: string, data: any): void {
      ...
      if (clientSocket) {
        ...
        this.logger.debug(`[${EventsGateway.name}] Event "${eventName}" sent to client ${socketId}`);
      } else {
        this.logger.warn(`[${EventsGateway.name}] Client with socketId ${socketId} not found. Could not send event "${eventName}".`);
      }
    }
  
    @SubscribeMessage('message') // 예시: 클라이언트로부터 'message' 이벤트를 수신
    handleMessage(client: Socket, payload: any): void {
      this.logger.log(`[${EventsGateway.name}] Message from client ${client.id}: ${JSON.stringify(payload)}`);
