...
export class KeywordMappingService {
  ...
  getMapping(query: string): StockMapping {
    ...
    for (const mapping of stockMappings) {
      if (mapping.keywords.includes(lowerCaseQuery) || mapping.primaryName.toLowerCase() === lowerCaseQuery) {
        return mapping;
      }
    }
    ...
    for (const mapping of stockMappings) {
      if (mapping.keywords.some(keyword => lowerCaseQuery.includes(keyword)) ||
